...
class Product extends Model {
    ...
    public function insert(): ?int
    {
        ...
        if(!empty($this->sku)) {
            ...
            if($stmt->rowCount() > 0) {
                $this->message = "SKU já existe!";
                return false;
            }
        }

        $query = "INSERT INTO products (club_id, name, description, price, stock, category_id, fandom, rarity, sku, is_physical, subscription_only, weight_grams, dimensions_cm, image_url, is_active) 
                  VALUES (:club_id, :name, :description, :price, :stock, :category_id, :fandom, :rarity, :sku, :is_physical, :subscription_only, :weight_grams, :dimensions_cm, :image_url, :is_active)";
    ...
    public function update(): bool
    {
        ...
        if(!empty($this->sku)) {
            $query = "SELECT * FROM products WHERE sku = :sku AND id != :id";
            $stmt = $conn->prepare($query);
            $stmt->bindParam(":sku", $this->sku);
            ...
            if($stmt->rowCount() > 0) {
                $this->message = "SKU já existe em outro produto!";
                return false;
            }
        }
        ...
        $query = "UPDATE products 
                  SET name = :name, description = :description, price = :price, stock = :stock, 
                      category_id = :category_id, fandom = :fandom, rarity = :rarity, sku = :sku, 
